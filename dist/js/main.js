			...
			if(query_detect[k].type == 'unique'){
				...
				for (var j in target){
					if (target.hasOwnProperty(j)) {
						if(target[j].retro_text !== '' && target[j].retro_text !== undefined){ target[j].retro_text = '#'+'<p><span style\\="background-color: '+target[j].color+'; color: white; padding-left: 8px; padding-right: 8px">'+target[j].retro_text+'</span></p>'; }
